{ lib, callPackage, iosevka-bin, nerd-font-patcher,
  variant ? "", symbol-fonts ? ["powerline" "fontawesome"], extra-option ? "", ... }:

let patcher = "${nerd-font-patcher}/bin/nerd-font-patcher";
    symbol-options = lib.strings.concatMapStringsSep " " (x: "--" + x) symbol-fonts;
in (iosevka-bin.override {variant = variant;}).overrideAttrs (final: prev: {
  pname = prev.pname + "-nf";
